...
serve_read(envid_t envid, union Fsipc *ipc)
{
    ...
    /* Look up the open-file structure for this <envid,fileid> pair. */
    struct OpenFile *o;
    int r = openfile_lookup(envid, req->req_fileid, &o);
    if (r < 0)
        return r;                    /* -E_BAD_ENV / -E_INVAL */

    /* Bound the copy so it fits in the reply page. */
    size_t n = req->req_n;
    if (n > sizeof(ret->ret_buf))
        n = sizeof(ret->ret_buf);

    /* Perform the read starting at the caller’s current offset. */
    r = file_read(o->o_file, ret->ret_buf, n, o->o_fd->fd_offset);
    if (r < 0)
        return r;                    /* propagate error */

    /* Advance seek position by the amount actually returned. */
    o->o_fd->fd_offset += r;

    if (debug)
        cprintf("serve_read: returned %d bytes\n", r);

    return r;                        /* becomes the IPC return value */
}
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	if (debug)
		cprintf("serve_write: returning %d bytes\n", r);
	//cant use ipc
	req->req_buf[PGSIZE - 1] = '\0'; // Ensure null-termination
	if (debug)
		cprintf("serve_write: returning %d bytes, buf %s\n", r, req->req_buf);
	return r;
