...
static void
debug_dump_elf(const char *prog, int fd)
{
    struct Elf ehdr;
    struct Proghdr phdr;
    ...
    // 1) Show the first 16 raw bytes
    if ((r = seek(fd, 0)) < 0) {
        cprintf("debug: seek to 0 failed: %e\n", r);
        return;
    }
    if ((r = readn(fd, magic, sizeof(magic))) < 0) {
        cprintf("debug: read magic failed: %e\n", r);
        return;
    }
    cprintf("debug: %s first 16 bytes:", prog);
    for (i = 0; i < 16; i++)
        cprintf(" %02x", magic[i]);
    cprintf("\n");

    // 2) Read and decode the ELF header
    if ((r = seek(fd, 0)) < 0 ||
        (r = readn(fd, &ehdr, sizeof ehdr)) != sizeof ehdr) {
        cprintf("debug: failed to read Elf header: %e/%d\n", r, r);
        return;
    }
    cprintf("debug: ELF header:\n");
    cprintf("       e_magic    = 0x%08x\n", ehdr.e_magic);
    cprintf("       e_entry    = 0x%08x\n", ehdr.e_entry);
    cprintf("       e_phoff    = %u\n",  ehdr.e_phoff);
    cprintf("       e_phnum    = %u\n",  ehdr.e_phnum);
    cprintf("       e_phentsize= %u\n",  ehdr.e_phentsize);

    // 3) Dump each program header
    for (i = 0; i < ehdr.e_phnum; i++) {
        uintptr_t off = ehdr.e_phoff + i * ehdr.e_phentsize;
        if ((r = seek(fd, off)) < 0 ||
            (r = readn(fd, &phdr, sizeof phdr)) != sizeof phdr) {
            cprintf("debug: read phdr[%d] failed: %e/%d\n", i, r, r);
            return;
        }
        cprintf("debug: PHDR[%d]: type=%u  off=0x%08x  vaddr=0x%08x\n"
    ...
    // rewind so spawn() can read again from offset zero
    seek(fd, 0);
}
...
spawn(const char *prog, const char **argv)
{
	...
	fd = r;
	//dump elf header
// cprintf("spawn opening %s → fd %d\n", prog, fd);
  //  debug_dump_elf(prog, fd);
...
copy_shared_pages(envid_t child)
{
	...
	for (addr = 0; addr < UTOP; addr += PGSIZE) {
		...
		/* Skip entire PDX regions that aren’t present to speed things up. */
		if (!(uvpd[PDX(addr)] & PTE_P)) {
			addr = (addr & ~(PGSIZE * NPTENTRIES - 1))          /* round to start of next PD */
			     +  (PGSIZE * NPTENTRIES) - PGSIZE;
			continue;
		}

		/* Only interested in present pages that are marked PTE_SHARE.   */
		if ((uvpt[PGNUM(addr)] & (PTE_P | PTE_SHARE)) ==
		    (PTE_P | PTE_SHARE)) {
